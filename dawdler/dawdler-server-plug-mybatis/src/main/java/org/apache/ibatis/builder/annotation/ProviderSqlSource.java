...
public class ProviderSqlSource implements SqlSource {
	 ...
	 * @deprecated Since 3.5.3, Please use the
	 *             {@link #ProviderSqlSource(Configuration, Annotation, Class, Method)}
	 *             instead of this.
	 */
	@Deprecated
	 ...
	 * @since 3.5.3
	 */
	public ProviderSqlSource(Configuration configuration, Annotation provider, Class<?> mapperType,
			Method mapperMethod) {
		...
		try {
			...
			if (candidateProviderMethodName.length() == 0
				...
				candidateProviderMethod = ((ProviderMethodResolver) this.providerType.getDeclaredConstructor()
						.newInstance()).resolveMethod(
								new ProviderContext(mapperType, mapperMethod, configuration.getDatabaseId()));
			}
					...
					if (candidateProviderMethodName.equals(m.getName())
						...
						if (candidateProviderMethod != null) {
							throw new BuilderException("Error creating SqlSource for SqlProvider. Method '"
									+ candidateProviderMethodName + "' is found multiple in SqlProvider '"
		...
		if (candidateProviderMethod == null) {
			throw new BuilderException(
					"Error creating SqlSource for SqlProvider. Method '" + candidateProviderMethodName
		...
		try {
			...
			if (parameterObject instanceof Map) {
				...
				throw new BuilderException("Cannot invoke SqlProvider method '" + providerMethod
						...
						+ "' because SqlProvider method arguments for '" + mapperMethod
