        ...
        try {
            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < fields.length; i++) {
                    ...
                    Method setMethod = clazz.getMethod(setMethodName, fields[i].getType());
                    try {
                        setMethod.invoke(object, rs.getObject(fieldName.toLowerCase()));
                    } catch (Exception e) {
                        try {
                            setMethod.invoke(object, rs.getString((fieldName.toLowerCase())));
                        } catch (Exception e1) {
                            try {
                                Class c = fields[i].getType();
                                if (c.isAssignableFrom(int.class) || c.isAssignableFrom(Integer.class)) {
                                    setMethod.invoke(object, rs.getInt((fieldName.toLowerCase())));
                                } else if (c.isAssignableFrom(short.class) || c.isAssignableFrom(Short.class)) {
                                    setMethod.invoke(object, rs.getShort((fieldName.toLowerCase())));
                                } else if (c.isAssignableFrom(long.class) || c.isAssignableFrom(Long.class)) {
                                    setMethod.invoke(object, rs.getLong((fieldName.toLowerCase())));
                                } else if (c.isAssignableFrom(byte.class) || c.isAssignableFrom(Byte.class)) {
                                    setMethod.invoke(object, rs.getByte((fieldName.toLowerCase())));
                                } else if (c.isAssignableFrom(byte[].class) || c.isAssignableFrom(Byte[].class)) {
                                    ...
                                    setMethod.invoke(object, rs.getDouble((fieldName.toLowerCase())));
                                } else if (c.isAssignableFrom(float.class) || c.isAssignableFrom(Float.class)) {
                                    setMethod.invoke(object, rs.getFloat((fieldName.toLowerCase())));
                                }
                            } catch (Exception e2) {
                            }
                        }
