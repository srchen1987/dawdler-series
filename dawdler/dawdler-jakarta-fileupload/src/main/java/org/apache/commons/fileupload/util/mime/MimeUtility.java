...
public final class MimeUtility {
	...
	/**
	 * Decode a string of text obtained from a mail header into its proper form. The
	 * text generally will consist of a string of tokens, some of which may be
	...
	public static String decodeText(String text) throws UnsupportedEncodingException {
		...
		while (offset < endOffset) {
			...
			if (LINEAR_WHITESPACE.indexOf(ch) != -1) { // whitespace found
				...
				// this is a normal token, so it doesn't matter what the previous token was. Add
				// the white space
				// if we have it.
	...
	private static String decodeWord(String word) throws ParseException, UnsupportedEncodingException {
		// encoded words start with the characters "=?". If this not an encoded word, we
		// throw a
