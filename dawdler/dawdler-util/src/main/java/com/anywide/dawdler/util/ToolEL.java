...
public class ToolEL {
	...
	private static int getType(Object o){
		...
		if(type.isArray()){
			if(type.isAssignableFrom(String[].class))return STRINGTYPE;
			if(type.isAssignableFrom(byte[].class)||type.isAssignableFrom(Byte[].class))return BYTETYPE;
			if(type.isAssignableFrom(short[].class)||type.isAssignableFrom(Short[].class))return SHORTTYPE;
			if(type.isAssignableFrom(int[].class)||type.isAssignableFrom(Integer[].class))return INTTYPE;
			if(type.isAssignableFrom(long[].class)||type.isAssignableFrom(Long[].class))return LONGTYPE;
			if(type.isAssignableFrom(float[].class)||type.isAssignableFrom(Float[].class))return FLOATTYPE;
			if(type.isAssignableFrom(double[].class)||type.isAssignableFrom(Double[].class))return DOUBLETYPE;
			if(type.isAssignableFrom(char[].class)||type.isAssignableFrom(Character[].class))return CHARTYPE;
			if(type.isAssignableFrom(boolean[].class)||type.isAssignableFrom(Boolean[].class))return BOOLEANTYPE;
			return OBJECTTYPE;
			}
		if(o instanceof List)return LISTTYPE;
		if(o instanceof Map)return MAPTYPE;
		if(o instanceof Set)return SETYPE;
		if(o instanceof Enumeration)return ENUETYPE;
		return BASETYPE;
	}
}
