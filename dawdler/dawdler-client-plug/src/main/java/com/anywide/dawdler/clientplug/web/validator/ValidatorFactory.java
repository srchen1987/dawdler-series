...
public class ValidatorFactory {
	...
	private static Map<String,AbstractValidator> validators = Collections.synchronizedMap(new HashMap<String,AbstractValidator>());
	public static AbstractValidator getValidator(String key){
		try {
			AbstractValidator avalidator=validators.get(key);
			if(avalidator!=null)return avalidator;
			   avalidator= new AbstractValidator(key) { 
			};
			validators.put(key, avalidator);
			return avalidator;
		} catch (Exception e) {
			logger.error("",e);
			return null;
