...
| @DateTimeFormat | 参数 | 获取日期参数,其中value为日期参数名 | 支持Date、LocalDateTime、LocalDate、LocalTime、ZonedDateTime、OffsetTime、OffsetDateTime |

DateTimeFormat注解详细说明:
...
iso设置为NONE(默认)时 会按指定的pattern进行格式化.
...
| Date | LocalDateTime | LocalDate | LocalTime | ZonedDateTime | OffsetTime | OffsetDateTime |
| :-: | :-: | :-: | :-: | :-: | :-: | :-: |

说明: 时间类型如果采用@DateTimeFormat注解请参考@DateTimeFormat对应的文档.

如果没有@DateTimeFormat默认按以下格式进行转换:

Date、LocalDateTime、ZonedDateTime、OffsetDateTime 默认格式为: yyyy-MM-dd HH:mm:ss

LocalDate 默认格式为: yyyy-MM-dd

LocalTime、 OffsetTime 默认格式为: HH:mm:ss

更改默认格式 通过启动变量配置:

dawdler.datetime.format=yyyy-MM-dd HH:mm:ss

dawdler.date.format=yyyy-MM-dd

dawdler.time.format=HH:mm:ss
