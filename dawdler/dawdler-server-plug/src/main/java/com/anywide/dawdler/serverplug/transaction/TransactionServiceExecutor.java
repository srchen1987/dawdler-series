...
import com.anywide.dawdler.serverplug.annotation.DBTransaction.READCONFIG;
...
public class TransactionServiceExecutor implements ServiceExecutor {
	private static Logger logger = LoggerFactory.getLogger(TransactionServiceExecutor.class);
		...
		try {
			if (dbt != null && dbt.useConnection()) {
				...
				RWSplittingDataSourceManager dm = (RWSplittingDataSourceManager) context
						.getAttribute(RWSplittingDataSourceManager.DATASOURCEMANAGER_PREFIX);
				MODE mode = dbt.mode();
