		...
		try {
			...
			synchronized (loader) {
				try {
					...
					MethodVisitor mv;
					cw.visit(V25, ACC_PUBLIC + ACC_SUPER, accessClassNameInternal, null,
							"club/dawdler/util/reflectasm/MethodAccess", null);
					{
						mv = cw.visitMethod(ACC_PUBLIC, "<init>", "()V", null, null);
						mv.visitCode();
						mv.visitVarInsn(ALOAD, 0);
						mv.visitMethodInsn(INVOKESPECIAL, "club/dawdler/util/reflectasm/MethodAccess", "<init>",
								"()V", false);
						mv.visitInsn(RETURN);
						mv.visitMaxs(0, 0);
						mv.visitEnd();
