...
public class SelectorContext implements Context {
	 ...
	 *                                                           mandatory
	 *                                                           attributes
	 * @throws NamingException                                   if a naming
	 *                                                           exception is
	 *                                                           encountered
	 */
	@Override
	 ...
	 * <p>
	 * If the object is a DirContext, any existing attributes associated with the
	 * name are replaced with those of the object. Otherwise, any existing
	 ...
	 *
	 * @param name the name to bind; may not be empty
	 * @throws javax.naming.NameNotFoundException if an intermediate context does
	 *                                            not exist
	 ...
	 * name. Both names are relative to this context. Any attributes associated with
	 * the old name become associated with the new name. Intermediate contexts of
	 ...
	 * the intermediate contexts do not exist.
	 * <p>
	 * In a federated naming system, a context from one naming system may be bound
	 * to a name in another. One can subsequently look up and perform operations on
	 * the foreign context using a composite name. However, an attempt destroy the
	 * context using this composite name will fail with NotContextException, because
	 * the foreign context is not a "subcontext" of the context in which it is
	 * bound. Instead, use unbind() to remove the binding of the foreign context.
	 * Destroying the foreign context requires that the destroySubcontext() be
	 * performed on a context from the foreign context's "native" naming system.
	 *
	 * @param name the name of the context to be destroyed; may not be empty
	 * @throws javax.naming.NameNotFoundException if an intermediate context does
	 *                                            not exist
	 * @throws javax.naming.NotContextException   if the name is bound but does not
	 *                                            name a context, or does not name a
	 *                                            context of the appropriate type
	 */
	@Override
	...
	/**
	 * Creates and binds a new context. Creates a new context with the given name
	 * and binds it in the target context (that named by all but terminal atomic
	 * component of the name). All intermediate contexts and the target context must
	 * already exist.
	 *
	 ...
	 * namespaces, different naming systems will parse names differently. This
	 * method allows an application to get a parser for parsing names into their
	 * atomic components using the naming convention of a particular naming system.
	 * Within any single naming system, NameParser objects returned by this method
	 * must be equal (using the equals() test).
	 *
	 ...
	 * <p>
	 * Given a name (name) relative to this context, and the name (prefix) of this
	 * context relative to one of its ancestors, this method returns the composition
	 * of the two names using the syntax appropriate for the naming system(s)
	 * involved. That is, if name names an object relative to this context, the
	 * result is the name of the same object, but relative to the ancestor context.
	 ...
	 * property already exists, its value is overwritten.
	 *
	 ...
	 *
	 * @param propName the name of the environment property to remove; may not be
	 *                 null
	 * @throws NamingException if a naming exception is encountered
	 */
	@Override
	...
	/**
	 * Retrieves the environment in effect for this context. See class description
	 * for more details on environment properties. The caller should not make any
	 * changes to the object returned: their effect on the context is undefined. The
	 * environment of this context may be changed using addToEnvironment() and
	 ...
	 * immediately, instead of waiting for them to be released automatically by the
	 * garbage collector. This method is idempotent: invoking it on a context that
	 * has already been closed has no effect. Invoking any other method on a closed
	 ...
	 * application to retrieve this name. The string returned by this method is not
	 * a JNDI composite name and should not be passed directly to context methods.
	 ...
	 * @throws NamingException if there is no "java:" header or if no naming context
	 *                         has been bound to this thread
	 */
