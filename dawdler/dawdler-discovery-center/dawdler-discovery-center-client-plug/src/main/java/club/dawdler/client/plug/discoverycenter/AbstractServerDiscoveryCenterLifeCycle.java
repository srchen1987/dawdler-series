/*
 * Licensed to the Apache Software Foundation (ASF) under one or more
 * contributor license agreements.  See the NOTICE file distributed with
 * this work for additional information regarding copyright ownership.
 * The ASF licenses this file to You under the Apache License, Version 2.0
 * (the "License"); you may not use this file except in compliance with
 * the License.  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package club.dawdler.client.plug.discoverycenter;

import java.util.Map;
import java.util.concurrent.TimeUnit;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import club.dawdler.clientplug.web.conf.WebApplicationConfig;
import club.dawdler.clientplug.web.conf.WebConfigParser;
import club.dawdler.core.component.resource.ComponentLifeCycle;
import club.dawdler.core.discoverycenter.DiscoveryCenter;
import club.dawdler.util.HashedWheelTimer;
import club.dawdler.util.Timeout;
import club.dawdler.util.TimerTask;

/**
 * @author jackson.song
 * @version V1.0
 * web端注册中心抽象类
 */
public abstract class AbstractServerDiscoveryCenterLifeCycle implements ComponentLifeCycle {
	private static final Logger logger = LoggerFactory.getLogger(AbstractServerDiscoveryCenterLifeCycle.class);
	private Timeout timeout;
	private long checkTime = 5000;
	private HashedWheelTimer hashedWheelTimer;
	protected WebApplicationConfig webApplicationConfig = null;

	public AbstractServerDiscoveryCenterLifeCycle() {
		if (WebConfigParser.getWebConfig() != null) {
			webApplicationConfig = WebConfigParser.getWebConfig().getWebApplicationConfig();
		}
	}

	public void addProvider(Map<String, Object> attributes) throws Exception {
		if (webApplicationConfig != null) {
			DiscoveryCenter discoveryCenter = getDiscoveryCenter();
			String path = webApplicationConfig.getName();
			String host = webApplicationConfig.getHost();
			int port = webApplicationConfig.getPort();
			String value = host + ":" + port;
			if (discoveryCenter.addProvider(path, value, attributes)) {
				logger.info("add web service {} on {}", path, value);
			}
			hashedWheelTimer = new HashedWheelTimer();
			timeout = hashedWheelTimer.newTimeout(new ProviderTimeoutTask(path, value, attributes), checkTime,
					TimeUnit.MILLISECONDS);
		}
	}

	@Override
	public void prepareDestroy() throws Throwable {
		if (webApplicationConfig == null) {
			return;
		}
		String path = webApplicationConfig.getName();
		String host = webApplicationConfig.getHost();
		int port = webApplicationConfig.getPort();
		String value = host + ":" + port;
		if (timeout != null) {
			timeout.cancel();
		}
		if (hashedWheelTimer != null) {
			hashedWheelTimer.stop();
		}
		DiscoveryCenter discoveryCenter = getDiscoveryCenter();
		if (discoveryCenter != null) {
			discoveryCenter.deleteProvider(path, value);
			discoveryCenter.destroy();
		}

	}

	public class ProviderTimeoutTask implements TimerTask {
		private String path;
		private String value;
		private Map<String, Object> attributes;

		public ProviderTimeoutTask(String path, String value, Map<String, Object> attributes) {
			this.path = path;
			this.value = value;
			this.attributes = attributes;
		}

		@Override
		public void run(Timeout timeout) throws Exception {
			if (timeout.isCancelled()) {
				return;
			}
			try {
				DiscoveryCenter discoveryCenter = getDiscoveryCenter();
				if (!discoveryCenter.isExist(path, value)) {
					discoveryCenter.addProvider(path, value, attributes);
				}
			} catch (Exception e) {
				logger.error("", e);
			}
			if (hashedWheelTimer.getWorkerState().get() == HashedWheelTimer.WORKER_STATE_STARTED) {
				AbstractServerDiscoveryCenterLifeCycle.this.timeout = timeout.timer().newTimeout(this, checkTime,
						TimeUnit.MILLISECONDS);
			}
		}

	}

	public abstract DiscoveryCenter getDiscoveryCenter() throws Exception;
}
