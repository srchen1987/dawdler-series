...
//		String explain="parentname=,childname=,parentvalue=,remark=";
		...
		for(String value:values){
			...
			if(vs.length!=2)throw new ParseErrorException("must like explain for example var=tempdata,parentname=id,childname=parendid,parentvalue=0,remark=- ");
		...
		for(Iterator it = item.iterator();it.hasNext();){
			...
			if(ko==null)throw new ParseErrorException("no find "+childname+" propertie in "+o.getClass().getName());
...
//		StringBuffer sb = new StringBuffer();
	...
	private void createTree(Map<String,List> data,List targetdata,String parentname,String parentvalue,String appendremark,String remark){
...
//		appendremark.append(remark);
		...
		for(Object o: list){
			...
			boolean temp=false;
		    if(data.get(key)==null)temp=true;
		    targetdata.add(new Object[]{appendremark,o,temp});
			createTree(data,targetdata, parentname,key.toString(),appendremark,remark);
		}
	}
	/*public static void main(String[] args) throws ResourceNotFoundException, ParseErrorException, MethodInvocationException, IOException {
		TreeDirect td = new TreeDirect();
		td.render(null, null, null);
	}*/
}
