	...
	public void smartRegister(Class<?> service)throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		...
		if(remoteServiceContract!=null){
			...
			if (StringUtils.isBlank(name)) {
				registerService(service,SunReflectionFactoryInstantiator.newInstance(service),remoteServiceContract.single()); 
			} else {
...
//			}
			if (interfaceList != null) {
				for (Class<?> clazz : interfaceList) {
					...
					String name = remoteServiceContract.value();
					if (StringUtils.isBlank(name)) {
						registerService(clazz,SunReflectionFactoryInstantiator.newInstance(service),remoteServiceContract.single());
					} else {
						register(name,SunReflectionFactoryInstantiator.newInstance(service),remoteServiceContract.single());
					}
				}
