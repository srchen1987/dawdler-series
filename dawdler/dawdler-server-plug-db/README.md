...
```xml
<decisions>
  <decision mapping="com.anywide.yyg.user.service.impl" latent-expression-id="userDataSource" />
  <!-- mapping为指定的包,指定的包下所有的service实现类都会采用latent-expression-id所配置的规则,优先级高于antpath匹配规则 -->
  <decision mapping="com.anywide.yyg.**.service.impl" latent-expression-id="userDataSource" />
